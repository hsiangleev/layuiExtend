...
layui.define(['laytpl', 'laypage', 'layer', 'form'], function(exports){
					...
					,'{{# layui.each(item1, function(i2, item2){ }}'
						...
						,'<th data-field="{{ item2.field||i2 }}" {{# if(item2.minWidth){ }}data-minwidth="{{item2.minWidth}}"{{# } }} '+ rowCols +' {{# if(item2.unresize){ }}data-unresize="true"{{# } }}>'
							...
							,'" {{#if(item2.align){}}align="{{item2.align}}"{{#}}}>'
								...
								,'{{# } else if(item2.type === "radio") { }}'//单选框lee
		...
		Class.prototype.initOpts = function(item){
			...
			if(item.radio) item.type = "radio";			// lee
		...
		Class.prototype.events = function(){
			...
			//单元行双击事件lee
			that.layBody.on('dblclick', 'tr', function(){
				var othis = $(this);
				var index=othis.data('index');
				var data=table.cache[that.key] ? (table.cache[that.key][index] || {}) : {};

				that.config.dblclick?(function() {
					that.config.dblclick(data);
				})():"";
			});
